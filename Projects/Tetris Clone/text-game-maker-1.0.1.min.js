...
var TM = {
    ...
    defaultSettings: {
        ...
        charGroups: {korean: {chars: "ㄱ-힝", isFullwidth: !0, sizeAdj: 1, xAdj: 0, yAdj: 0}},
        debug: {devMode: !1, outputDomId: "tm-debug-output"}
    },
    common: {}
};
TM.common.getBlockWidth = function (t) {
    return .6 * t
}, TM.common.getBlockHeight = function (t) {
    var e;
    if (t < 3) e = t; else {
        var r = [6, 7, 7], n = 0, o = 1, i = function (t) {
...
}, TM.common.includeScript = function (t) {
    if (t) {
        var e = document.querySelector('script[src="' + t + '"]');
        e || ((e = document.createElement("script")).src = t, document.getElementsByTagName("head")[0].appendChild(e))
    }
}, TM.common.mergeObjects = function (t, e) {
    for (var r = {}, n = [t, e], o = 0; o < n.length; o++) for (var i in n[o]) r[i] = n[o][i];
    return r
    ...
    for (var t in TM.ILoopObject.prototype.inactivate.call(this), this.objects) if (Array.isArray(this.objects[t])) for (var e = this.objects[t].length - 1; 0 <= e; e--) this.objects[t][e].inactivate(); else this.objects[t] && this.objects[t].inactivate()
...
}, TM.IProgram.prototype._timeline = function (t) {
}, TM.IProgram.prototype._getInput = function () {
}, TM.ScreenManager_Char = function (t, e, r, n, o) {
...
}, TM.ScreenManager_Char.prototype = Object.create(TM.IObject.prototype), TM.ScreenManager_Char.prototype.constructor = TM.ScreenManager_Char, TM.ScreenManager_Char.prototype._init = function () {
}, TM.ScreenManager_Char.prototype._inactivate = function () {
}, TM.ScreenManager_Char.prototype.update = function (t, e, r, n) {
...
}, TM.ScreenManager_Cursor.prototype.move = function (t, e) {
    var r = !1;
    if (0 <= t && t <= this.data.xMax && 0 <= e && e <= this.data.yMax) {
        var n = this.data.refScreenManager;
        r = n.screenData[this.data.y + n.scrollOffsetY][this.data.x].isNew = !0, this.data.x = t, this.data.y = e
    }
    return r
    ...
    this.resetScreenData(), this.screenSetting.fontSource ? this.startLoadingFont() : this.isFontLoaded = !0
}, TM.ScreenManager.prototype._inactivate = function () {
}, TM.ScreenManager.prototype._calculate = function () {
    this.checkReady() ? this.onReadyFunc && (this.onReadyFunc(), delete this.onReadyFunc) : this.drawSystemMessage("Loading...")
}, TM.ScreenManager.prototype._draw = function () {
    var t = this.ctx;
    t.textBaseline = "buttom";
    for (var e = this.getNewBgUpdateMap(), r = this.scrollOffsetY; r < this.scrollOffsetY + this.screenSetting.row; r++) for (var n = 0; n < this.screenSetting.column; n++) {
        if (!0 === this.screenData[r][n].isNew) {
            if (!e[r][n]) {
                var o = this.blockWidth * n, i = this.blockHeight * (r - this.scrollOffsetY),
                    s = this.getBackgroundWidthRecursive(r, n, e), a = this.blockHeight;
                t.fillStyle = this.screenData[r][n].backgroundColor, t.fillRect(o, i, s, a)
            }
            if (this.screenData[r][n].char && "$" != this.screenData[r][n].char[0]) {
                var c = this.blockWidth * n, h = this.blockHeight * (r - this.scrollOffsetY) + .8 * this.blockHeight,
                    p = TM.common.getCharGroup(this.charGroups, this.screenData[r][n].char);
                p ? (t.font = this.screenSetting.fontSize * p.sizeAdj + "px " + this.screenData[r][n].font, c += this.blockWidth * p.xAdj, h += this.blockHeight * p.yAdj) : t.font = this.screenSetting.fontSize + "px " + (this.isFontLoaded ? this.screenData[r][n].font : "monospace"), t.fillStyle = this.screenData[r][n].color, t.fillText(this.screenData[r][n].char[0], c, h)
            }
        ...
        if (!this.screenData[r][n].isActive && "$off" != this.screenData[r][n].char) {
            ...
            var u = this.blockWidth * n, l = this.blockHeight * (r - this.scrollOffsetY), d = this.blockWidth,
                g = this.blockHeight;
            t.fillStyle = this.screenData[r][n].backgroundColor, t.fillRect(u, l, d, g)
        }
    }
    var M = this.cursor.data;
    if (M.isUpdated) if (M.isUpdated = !1, M.isHidden) this.screenData[M.y + this.scrollOffsetY][M.x].isNew = !0; else {
        var f = M.width, y = this.blockHeight * M.size, T = this.blockWidth * M.x,
            S = this.blockHeight * M.y + (this.blockHeight - y), v = this.screenSetting.fontColor;
        t.fillStyle = v, t.fillRect(T, S, f, y)
    }
}, TM.ScreenManager.prototype.startLoadingFont = function (t) {
    var e = t || this;
    if (!e.screenSetting.webFontJsPath) return console.error("TM.ScreenManager ERROR: 'webFontJsPath' is required to load font from 'fontSource'!");
    if (!e.screenSetting.fontFaceObserverJsPath) return console.error("TM.ScreenManager ERROR: 'fontFaceObserverJsPath' is required to load font from 'fontSource'!");
    var r = !0;
    window.WebFont || (r = !1, TM.common.includeScript(e.screenSetting.webFontJsPath)), window.FontFaceObserver || (r = !1, TM.common.includeScript(e.screenSetting.fontFaceObserverJsPath)), r ? e.loadWebFont() : setTimeout(function () {
        TM.ScreenManager.prototype.startLoadingFont(e)
    }, 100)
}, TM.ScreenManager.prototype.loadWebFont = function () {
    WebFont.load({custom: {families: [this.screenSetting.fontFamily], urls: [this.screenSetting.fontSource]}});
    var t = new FontFaceObserver(this.screenSetting.fontFamily), e = this;
    t.load().then(function () {
        e.isFontLoaded = !0, e.refreshScreen(), e.drawSystemMessage("Loading...", !0)
    }, function () {
        e.isFontLoaded = !0;
        var t = "TM.ScreenManager ERROR: Cannot load font: " + e.screenSetting.fontFamily;
        e.drawSystemMessage(t), console.error(t)
    })
    ...
    for (var t = 0; t < this.screenData.length; t++) for (var e = 0; e < this.screenData[t].length; e++) this.screenData[t][e].isNew = !0
}, TM.ScreenManager.prototype.isInScreen = function (t, e) {
    var r = !1;
    return 0 <= t && 0 <= e && e < this.screenSetting.row && t < this.screenSetting.column && (r = !0), r
}, TM.ScreenManager.prototype.insertChar = function (t, e, r) {
    var n = this.cursor.data.x, o = this.cursor.data.y, i = this.cursor.data.x,
        s = this.cursor.data.y + this.scrollOffsetY;
    if (this.isInScreen(n, o)) {
        if (this.screenData[s][i].char != t || this.screenData[s][i].color != (e || this.screenSetting.fontColor) || this.screenData[s][i].backgroundColor != (r || this.screenSetting.backgroundColor) || "$" == this.screenData[s][i].char[0] && this.screenData[s][i - 1].isNew) {
            var a = this.FullwidthRegex, c = !!a && a.test(t);
            this.screenData[s][i].update(t, c, e, r), this.isInScreen(n - 1, o) && (this.screenData[s][i - 1].draw = !0), this.isInScreen(n + (c ? 2 : 1), o) && (this.screenData[s][i + (c ? 2 : 1)].draw = !0)
        }
...
}, TM.ScreenManager.prototype.drawSystemMessage = function (t, e) {
    var r = this.cursor.data.x, n = this.cursor.data.y;
    e ? this.deleteTextAt(0, 0, t) : this.insertTextAt(0, 0, t), this.cursor.move(r, n)
    ...
    for (var n = this.scrollOffsetY; n < this.scrollOffsetY + this.screenSetting.row; n++) for (var o = 0; o < this.screenSetting.column; o++) this.screenData[n][o].update(t, !1, e, r)
    ...
    0 < this.scrollOffsetY && this.scrollOffsetY--, this.refreshScreen()
    ...
    for (var o = this.cursor.data.x, i = this.cursor.data, s = 0; s < t.length; s++) switch (t[s]) {
        case"\n":
        ...
        default:
            var a = !!n && n.test(t[s]);
            this.insertChar(t[s], e, r), a && (s++, this.insertChar("$fullwidthFiller", e, r))
    }
...
}, TM.InputManager_Keyboard.prototype.checkKeyState = function (t) {
...
},TM.InputManager.prototype = Object.create(TM.IObject.prototype),TM.InputManager.prototype.constructor = TM.InputManager,TM.InputManager.prototype._init = function () {
},TM.InputManager.prototype._inactivate = function () {
},TM.DebugManager = function (t) {
...
},TM.DebugManager.prototype = Object.create(TM.IObject.prototype),TM.DebugManager.prototype.constructor = TM.DebugManager,TM.DebugManager.prototype._init = function () {
},TM.DebugManager.prototype._inactivate = function () {
    this.deleteAll()
},TM.DebugManager.prototype.replacer = function () {
    var r = [];
    return function (t, e) {
        if (e && "object" == typeof e && -1 < r.indexOf(e)) return "[Circular]";
        return r.push(e), e
    }
